...
void main(){
    ...
    while(1){
        ...
        lovalue=ADRESL;
        hivalue=ADRESH;
        value=((unsigned int)hivalue<<2)+ ((unsigned int)lovalue>>6);  //Adding 16 bits of lovalue and hivalue
...
void set_flow()
{
    TRISA=0x04;         //Set A2 as input remaining as output
    TRISC=0x00;         //Set pin RC0 to RC7  as output
    TRISD=0x00;         //data flow through all pins in port d
    ADCON0=0x91;        //set the adc flow
    ADCON1=0x09;        //set to left justified 
